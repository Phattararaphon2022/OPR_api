...
namespace BPC_OPR
{
    ...
    public class ModuleProject : IModuleProject
    {
                    ...
                    foreach (cls_MTProjobmain jobmain in list_jobmain)
                    {
                        ...
                        foreach (cls_TRTimecard tmp in time_list)
                        {
                            if (tmp.projob_code.Equals(jobmain.projobmain_code))
                            {
                                ...
                                //-- 
                                if (tmp.timecard_daytype.Equals("L"))
                                    count_leave++;
                            }
                        }
        ...
        #region  MTSize
        public string getMTSize(BasicRequest req)
        {
            JObject output = new JObject();
            cls_SYSApilog log = new cls_SYSApilog();
            log.apilog_code = "PRO032.1";
            log.apilog_by = req.username;
            log.apilog_data = "all";
            ...
            try
            {
                ...
                List<cls_MTSize> list = controller.getDataByFillter(req.company_code, "");
                JArray array = new JArray();

                if (list.Count > 0)
                {
                    int index = 1;
                    foreach (cls_MTSize model in list)
                    {
                        JObject json = new JObject();
                        json.Add("company_code", model.company_code);

                        json.Add("size_id", model.size_id);
                        json.Add("size_code", model.size_code);
                        json.Add("size_name_th", model.size_name_th);
                        json.Add("size_name_en", model.size_name_en);
                        json.Add("modified_by", model.modified_by);
                        json.Add("modified_date", model.modified_date);
                        json.Add("index", index++);
                        array.Add(json);
        ...
        public string doManageMTSize(InputMTSize input)
        {
            JObject output = new JObject();
            var json_data = new JavaScriptSerializer().Serialize(input);
            var tmp = JToken.Parse(json_data);
            cls_SYSApilog log = new cls_SYSApilog();
            log.apilog_code = "PRO032.2";
            log.apilog_by = input.modified_by;
            log.apilog_data = tmp.ToString();
            ...
            try
            {
                ...
                model.size_id = Convert.ToInt32(input.size_id);
                model.size_code = input.size_code;
                model.size_name_th = input.size_name_th;
                model.size_name_en = input.size_name_en;
                model.modified_by = input.modified_by;
        ...
        public string doDeleteMTSize(InputMTSize input)
        {
            JObject output = new JObject();
            var json_data = new JavaScriptSerializer().Serialize(input);
            var tmp = JToken.Parse(json_data);
            cls_SYSApilog log = new cls_SYSApilog();
            log.apilog_code = "PRO032.3";
            log.apilog_by = input.modified_by;
            log.apilog_data = tmp.ToString();
            ...
            try
            {
                ...
                if (controller.checkDataOld(input.size_code, input.company_code))
                {
                    bool blnResult = controller.delete(input.size_code, input.company_code);
                ...
                {
                    string message = "Not Found Project code : " + input.size_code;
                    output["success"] = false;
                    output["message"] = message;
        ...
        public async Task<string> doUploadMTSize(string token, string by, string fileName, Stream stream, string com)
        {
            JObject output = new JObject();

            cls_SYSApilog log = new cls_SYSApilog();
            log.apilog_code = "PRO032.4";
            log.apilog_by = by;
            log.apilog_data = "Stream";
...
>>>>>>> aa74680fce09581169dba4558e18b71105228c98
        {
            JObject output = new JObject();
